...
static SHOW_VAR innodb_status_variables[]= {
  ...
  {"rows_deleted", &export_vars.rows.deleted, SHOW_SIZE_T},
  {"rows_inserted", &export_vars.rows.inserted, SHOW_SIZE_T},
  {"rows_read", &export_vars.rows.read, SHOW_SIZE_T},
  {"rows_updated", &export_vars.rows.updated, SHOW_SIZE_T},
	...
	int	lock_type)	/*!< in: lock type */
{
	...
	if (lock_type != F_UNLCK) {
		...
		*trx->detailed_error = 0;
		...
		export_vars.rows.mutex.wr_lock();
		export_vars.rows.deleted += rows.deleted;
		export_vars.rows.inserted += rows.inserted;
		export_vars.rows.read += rows.read;
		export_vars.rows.updated += rows.updated;
		export_vars.rows.mutex.wr_unlock();
		new (&rows) typeof(rows) {};
	}
