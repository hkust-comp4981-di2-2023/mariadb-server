...
template <typename T> bool List_log_event<T>::to_packet(String *packet)
{
  ...
  {
    uint32 out= 0;
    write_element(packet, &list[i], &out);
    p+= out;
  }
...
#if defined(HAVE_REPLICATION)
void Gtid_list_log_event::write_element(String *packet, rpl_gtid *element,
                                        uint32 *size)
{
...
Xid_list_log_event::Xid_list_log_event(THD *thd)
    : List_log_event(0, xid_cache_get_count(thd), (event_xid_t *) NULL)
{
  cache_type= EVENT_NO_CACHE;
  ...
  if (count)
    xid_cache_get_xids(thd, list, count);
}


void Xid_list_log_event::write_element(String *packet, event_xid_t *element,
                                       uint32 *size)
{
  int4store(packet, element->formatID);
  *size+= 4;
  int4store(packet, element->gtrid_length);
  *size+= 4;
  int4store(packet, element->bqual_length);
  *size+= 4;
  long data_length= element->bqual_length + element->gtrid_length;
  memcpy(packet, element->data, data_length);
  *size+= data_length;
}


void
Xid_list_log_event::pack_info(Protocol *protocol)
{
  char buf_mem[1024];
  String buf(buf_mem, sizeof(buf_mem), system_charset_info);
  uint32 i;
  ...
  for (i= 0; i < count; ++i)
  {
    ...
    /*
      TODO Optimize with serialize method of event_xid_t that appends to a
      String object
    */
    list[i].serialize();
    buf.append(list[i].buf, list[i].key_length());
